  ...
  <nav class="navbar navbar-expand-lg navbar-dark bg-primary shadow-sm sticky-top main-nav">
    <div class="container px-3 px-lg-4">
      ...
      <div class="d-flex d-lg-none align-items-center gap-3 ms-auto">
        <button class="icon-btn-mobile" @click="toggleMobileSearch" aria-label="Search">
      ...
      <div class="collapse navbar-collapse" id="mainDesktopNav">
        ...
        <div class="d-none d-lg-flex align-items-center gap-3">
          ...
          <RouterLink class="desktop-cart-btn position-relative me-1" to="/cart" title="Shopping Cart">
            <i class="bi bi-cart3 fs-4"></i>
            <span v-if="cartStore.cartCount > 0" class="badge-count-desktop">
              {{ cartStore.cartCount }}
            </span>
          </RouterLink>
          ...
          <div class="dropdown">
            <div class="user-trigger dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
              ...
              <div class="user-info-text d-none d-xl-flex">
                <span class="welcome-msg">{{ currentUser ? 'Welcome' : 'Hello,' }}</span>
                <span class="user-name-bold text-truncate">
                  {{ currentUser ? currentUser.email.split('@')[0] : 'Sign In' }}
                </span>
              </div>
            </div>

            <ul class="dropdown-menu dropdown-menu-end user-dropdown-card animate-pop border-0 shadow-lg">
              <template v-if="currentUser">
                <li class="user-card-header bg-light mb-2">
                  <i class="bi bi-person-circle fs-2 text-primary"></i>
                  <div class="ms-2 overflow-hidden">
                    <h6 class="mb-0 text-truncate small fw-bold">{{ currentUser.email }}</h6>
                    <span class="badge bg-success-soft text-success px-2 py-1" style="font-size: 0.6rem;">Verified Member</span>
                  </div>
                </li>
                ...
                <li v-if="isAdmin">
                  ...
                  <RouterLink class="dropdown-item text-warning fw-bold py-2" to="/admin">
              ...
              <template v-else>
                <li class="p-4 text-center">
                  ...
                  <p class="small text-muted mb-3">Sign in to track orders and sync your cart.</p>
    ...
    <aside class="side-menu" :class="{ 'open': mobileMenuOpen }">
      ...
      <div class="side-menu-body">
        <div class="list-group list-group-flush">
          ...
          <template v-if="currentUser">
            <RouterLink to="/profile" class="list-item" @click="closeMobileMenu"><i class="bi bi-person"></i> My Profile</RouterLink>
            <RouterLink to="/orders" class="list-item" @click="closeMobileMenu"><i class="bi bi-bag-check"></i> My Orders</RouterLink>
            <button class="list-item text-danger border-0 bg-transparent w-100 text-start" @click="handleLogout">
          ...
          <template v-else>
            <div class="px-4 py-3">
              ...
              <RouterLink to="/auth?mode=register" class="btn btn-outline-primary w-100 rounded-pill" @click="closeMobileMenu">Create Account</RouterLink>
            </div>
          </template>
        </div>
      </div>
      <div class="side-menu-footer p-3 border-top text-center bg-light">
        <small class="text-muted">© 2026 ShopHub Store</small>
      </div>
    </aside>

    <Transition name="slide-top">
      <div v-if="mobileSearchOpen" class="mobile-search-full bg-primary shadow-lg">
        <div class="container h-100 d-flex align-items-center gap-2">
          <input
            ...
            @keyup.enter="goSearch"
          >
          <button class="btn btn-warning fw-bold" @click="goSearch">Search</button>
...
const navLinks = [
  ...
  { name: 'Shop', path: '/products', icon: 'bi-bag' },
...
.search-box-desktop:focus-within { background: white; width: 260px; }
...
.desktop-cart-btn:hover { background: rgba(255, 255, 255, 0.2); transform: translateY(-2px); }
...
.user-dropdown-card { width: 280px; border-radius: 16px; padding: 0.5rem; margin-top: 15px; }
.user-card-header { padding: 1rem; display: flex; align-items: center; border-radius: 12px; }
...
.side-menu-head { padding: 1.5rem; color: white; display: flex; justify-content: space-between; align-items: center; }
...
.list-item:hover { background: #f0f7ff; color: #0d6efd; padding-left: 2rem; }
...
.burger-menu span { width: 24px; height: 2px; background: white; border-radius: 5px; }
...
.mobile-search-full {
  ...
  z-index: 1060; border-bottom: 1px solid rgba(255,255,255,0.2);
}

.slide-top-enter-active, .slide-top-leave-active { transition: all 0.3s ease; }
.slide-top-enter-from, .slide-top-leave-to { transform: translateY(-100%); opacity: 0; }
</style>
